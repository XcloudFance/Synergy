...
function choice_tools(e, type, flag) {
    ...
    for (var i = 0; i < type.length; i++) {
        if (flag) {
            if (i == e) { type[i].style.backgroundColor = "yellow"; } else { type[i].style.backgroundColor = "#ccc"; }
        } else {
            if (i == e) { type[i].style.borderColor = "white"; } else { type[i].style.borderColor = "black"; }
        }
...
function Clear() {
    ...
    cxt.strokeStyle = "#fff";
    cxt.fillStyle = "#fff";
    cxt.fillRect(0, 0, 880, 380); //注意这里不用clearRect()
    cxt.strokeStyle = "#000";
    cxt.fillStyle = "#000";
    cxt.closePath();
}
...
function Arc(e) {
    ...
    canvas.onmouseup = function(e) {
        ...
        cxt.beginPath();
        cxt.arc(startx, starty, r / 2, 0, 360, false);
        cxt.stroke();
        cxt.closePath();
        cxt.stroke();
    }
